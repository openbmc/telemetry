...
class ReportFactory : public interfaces::ReportFactory
{
  ...
  private:
                   ...
                   const api::SensorPaths& sensorPaths)
    {
        ...
        {
            sensors.push_back(
                core::SensorCache::make<dbus::Sensor>(std::move(id), bus));
        }

        return sensors;
    }

    static std::vector<std::unique_ptr<const dbus::Sensor::Id>>
                     ...
                     const api::SensorPaths& sensorPaths)
    {
        ...
        std::vector<std::unique_ptr<const dbus::Sensor::Id>> ids;
        ...
        for (const auto& [sensor, ifacesMap] : tree)
        {
            for (const auto& [service, ifaces] : ifacesMap)
            {
                ...
                {
                    ids.push_back(std::make_unique<const dbus::Sensor::Id>(
                        service, sensor));
                }
            }
        }
