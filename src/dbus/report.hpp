...
class Report : public std::enable_shared_from_this<Report>
{
  public:
    ...
    Report(std::shared_ptr<sdbusplus::asio::connection> bus,
           ...
           const std::shared_ptr<core::interfaces::JsonStorage> storage) :
        bus_(bus),
  ...
  private:
    ...
    static inline std::vector<ReadingParam>
        convert(const std::vector<std::shared_ptr<core::interfaces::Metric>>&
                    coreMetrics)
    {
        ...
        for (auto& metric : coreMetrics)
        {
            ...
            {
                sensorPaths.push_back(std::string(sensor->path()));
            }
    ...
    static inline std::vector<dbus::Reading> convert(
        const std::vector<core::interfaces::Report::ReadingView>& coreReadings)
    {
    ...
    std::shared_ptr<core::interfaces::JsonStorage> storage_;
    ...
    utils::signals::Connection onPersistencyChanged_;
};
