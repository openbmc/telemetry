    ...
    public std::enable_shared_from_this<DiscreteThreshold>
{
  public:
    DiscreteThreshold(
        ...
        std::optional<double> thresholdValue, std::string name);
    ~DiscreteThreshold();
  ...
  private:
    ...
    const std::optional<double> thresholdValue;
    const std::string name;

    struct ThresholdDetail
    {
        struct TriggeringContext
        {
            double value;
            uint64_t timestamp;
        };
        ...
        std::optional<TriggeringContext> triggeringContext;
        double startingValue;
        std::optional<double> previousValue;
        boost::asio::steady_timer timer;

        ThresholdDetail(const std::string& name,
                        std::optional<TriggeringContext> triggeringContext,
                        double startingValue,
                        std::optional<double> previousValue,
                        boost::asio::io_context& ioc) :
            sensorName(name),
            triggeringContext(triggeringContext), startingValue(startingValue),
            previousValue(previousValue), timer(ioc)
        {}
    };
    std::vector<ThresholdDetail> details;

    void startTimer(const std::string&,
                    std::optional<ThresholdDetail::TriggeringContext>&,
