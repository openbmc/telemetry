    ...
    const std::vector<LabeledSensorInfo>& labeledSensorsInfo) const
{
    ...
    if (isDiscrete)
    {
        const auto& labeledDiscreteThresholdParams =
            std::get<std::vector<discrete::LabeledThresholdParam>>(labeledThresholdParams);
        for (const auto& labeledThresholdParam : labeledDiscreteThresholdParams)
        {
            namespace ts = utils::tstring;
            std::vector<std::unique_ptr<interfaces::TriggerAction>> actions;

            std::string thresholdName = labeledThresholdParam.at_label<ts::UserId>();
            discrete::Severity severity = labeledThresholdParam.at_label<ts::Severity>();
    ...
    else
    {
        ...
        for (const auto& labeledThresholdParam : labeledNumericThresholdParams)
        {
            ...
            {
                actions.emplace_back(
                    std::make_unique<action::numeric::LogToJournal>(type, thresholdValue));
            }

            if (logToRedfish)
            {
                actions.emplace_back(
                    std::make_unique<action::numeric::LogToRedfish>(type, thresholdValue));
            }
