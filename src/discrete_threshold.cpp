                                      ...
                                      uint64_t timestamp, double value)
{
    auto& [sensorName, triggeringContext, startingValue, previousValue, timer] =
        getDetails(sensor);
    const bool firstReading = !previousValue.has_value();
    const bool dwell = triggeringContext.has_value();

    if (thresholdValue.has_value())
    {
        if (dwell && value != thresholdValue.value())
        {
            timer.cancel();
    ...
    else if (!firstReading)
    {
        ...
        else
        {
            ...
            {
                startTimer(sensorName, triggeringContext, timer);
                startingValue = previousValue.value();
            }
        }
    }
    ...
    boost::asio::steady_timer& timer)
{
    ...
    {
        commit(sensorName, triggeringContext.value().timestamp,
               triggeringContext.value().value);
    }
    else
    {
