...
template <class T>
class Property : public DbusObject
{
  public:
    Property(
    ...
    {
        return signal.connect(std::forward<F>(fun));
    }

    template <class F>
    void addValidator(F&& validator)
    {
        validators.emplace_back(std::forward<F>(validator));
    }
  ...
  private:
    ...
    signals::Signal<void(const Property<T>&)> signal;
