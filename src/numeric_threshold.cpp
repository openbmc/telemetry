                                     ...
                                     uint64_t timestamp, double value)
{
    ...
    {
        startTimer(sensor.id(), timestamp, value, dwell, timer);
    }

    prevValue = value;
}

void NumericThreshold::startTimer(const interfaces::Sensor::Id& id,
                                  uint64_t timestamp, double value, bool& dwell,
                                  boost::asio::steady_timer& timer)
{
    if (dwellTime == std::chrono::milliseconds::zero())
    {
        commit(id, timestamp, value);
    }
    else
    {
        dwell = true;
        timer.expires_after(dwellTime);
        timer.async_wait([this, id, timestamp, value,
...
void NumericThreshold::commit(const interfaces::Sensor::Id& id,
                              uint64_t timestamp, double value)
{
    for (const auto& action : actions)
    {
        action->commit(id, timestamp, value);
    }
}
