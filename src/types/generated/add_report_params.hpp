...
{
    return details::stringToEnum(details::convDataReportingType, str);
}

inline std::string reportingTypeToString(ReportingType v)
{
    return std::string(
        details::enumToString(details::convDataReportingType, v));
}
...
{
    return details::stringToEnum(details::convDataReportingUpdates, str);
}

inline std::string reportingUpdatesToString(ReportingUpdates v)
{
    return std::string(
        details::enumToString(details::convDataReportingUpdates, v));
}
...
{
    return details::stringToEnum(details::convDataOperationType, str);
}

inline std::string operationTypeToString(OperationType v)
{
    return std::string(
        details::enumToString(details::convDataOperationType, v));
}
...
{
    return details::stringToEnum(details::convDataCollectionTimescope, str);
}

inline std::string collectionTimescopeToString(CollectionTimescope v)
{
    return std::string(
        details::enumToString(details::convDataCollectionTimescope, v));
}
...
struct adl_serializer<generated::MetricParams>
{
    ...
    {
        if (auto prop = j.find("sensorPaths"); prop != j.end())
        {
            prop->get_to(value.sensorPaths);
        }
        else
        {
            json(json::array({})).get_to(value.sensorPaths);
        }
        ...
        if (auto prop = j.find("metadata"); prop != j.end())
        {
            prop->get_to(value.metadata);
        }
        else
        {
            json("").get_to(value.metadata);
        }
...
struct adl_serializer<generated::AddReportParams>
{
    static void from_json(const json& j, generated::AddReportParams& value)
    {
        ...
        if (auto prop = j.find("emitsReadingsUpdate"); prop != j.end())
        {
            prop->get_to(value.emitsReadingsUpdate);
        }
        else
        {
            json(false).get_to(value.emitsReadingsUpdate);
        }
        if (auto prop = j.find("logToMetricReportsCollection"); prop != j.end())
        {
            prop->get_to(value.logToMetricReportsCollection);
        }
        else
        {
            json(false).get_to(value.logToMetricReportsCollection);
        }
