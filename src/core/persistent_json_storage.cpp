...
PersistentStorage::PersistentStorage(const Resource& directory) :
    directory_(directory)
{}

void PersistentStorage::store(const Resource& subPath,
...
bool PersistentStorage::remove(const Resource& subPath)
{
    const auto path = directory_ / subPath;
    std::error_code ec;
...
std::optional<nlohmann::json>
    PersistentStorage::load(const Resource& subPath) const
{
    const auto path = directory_ / subPath;
...
std::vector<PersistentStorage::Resource> PersistentStorage::list(
    const PersistentStorage::Resource& subDirectory) const
{
