...
Report::Report(boost::asio::io_context& ioc,
    ...
    recordUpdates(deduceRecordUpdates(reportUpdatesIn, reportingTypeIn)),
...
ReportUpdates
    Report::deduceRecordUpdates(const ReportUpdates reportUpdatesIn,
    ...
    dbusIface->register_property_rw(
        ...
        [this](std::string newVal, const auto&) {
            ...
            if (reportingType != ReportingType::OnRequest)
            {
                ...
                {
                    recordUpdates = newValConverted;
                    return true;
                }
            }
            return false;
        },
        [this](const auto&) { return reportUpdatesToString(recordUpdates); });
...
{
    if (recordUpdates == ReportUpdates::Overwrite)
    {
        readingsBuffer.clear();
    }
    else if (recordUpdates == ReportUpdates::AppendStopWhenFull &&
    ...
    for (const auto& metric : metrics)
    {
        ...
        {
            if ((recordUpdates == ReportUpdates::AppendStopWhenFull ||
                 recordUpdates == ReportUpdates::Overwrite) &&
                readingsBuffer.isFull())
            {
                break;
            }
...
bool Report::storeConfiguration() const
{
    try
    {
        ...
        data["ReportUpdates"] = reportUpdatesToString(recordUpdates);
