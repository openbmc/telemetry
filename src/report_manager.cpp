...
void ReportManager::loadFromPersistent()
{
    ...
    for (const auto& path : paths)
    {
        ...
        // Guard the optional before dereferencing; tidy requires explicit
        // control flow.
        if (!data)
        {
            // Nothing to load; remove the broken record and continue.
            reportStorage->remove(path);
            continue;
        }
        const auto& j = *data; // bind once after the guard
