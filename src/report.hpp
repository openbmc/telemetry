...
class Report : public interfaces::Report
{
  public:
    Report(boost::asio::io_context& ioc,
           ...
           uint64_t appendLimitIn, const ReportUpdates recordUpdatesIn,
  ...
  private:
    ...
    uint64_t deduceAppendLimit(
        const uint64_t appendLimitIn, const ReportUpdates recordUpdatesIn,
        const ReportingType reportingTypeIn,
        const std::vector<std::shared_ptr<interfaces::Metric>>& metricsIn)
        const;
    ReportUpdates
        deduceRecordUpdates(const ReportUpdates recordUpdatesIn,
