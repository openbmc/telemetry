...
class ReportTest : public Test
{
  public:
    ...
    {
        Metrics result;
        std::transform(std::begin(metricMocks), std::end(metricMocks),
                       std::back_inserter(result),
                       [](const auto& m) -> Metrics::value_type { return m; });
        return result;
    }
    ...
    std::shared_ptr<StorageMock> storageMock =
        std::make_shared<StrictMock<StorageMock>>();
       ...
       readingsContainAllReadingDoneByMetrics)
{
    ...
    std::vector<interfaces::Report::Reading> expected;
    for (size_t i = 0; i < metricMocks.size(); ++i)
    {
        expected.emplace_back(interfaces::Report::Reading(
