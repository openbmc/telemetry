#include "../actions.hpp"
#include "../core/mocks/json_storage.hpp"
...
class DBusReportTest : public Test
{
  public:
    ...
    {
        EXPECT_CALL(*storage,
                    store(core::interfaces::JsonStorage::Resource(
                              "Report/Domain/Name/configuration.json"),
                          _));

        sut = std::make_shared<decltype(sut)::element_type>(
            bus, objServer, coreReport, core::ReportName("Domain", "Name"),
            storage);
    }
    ...
    void SetUp() override
    {
        ...
        for (size_t i = 0; i < metrics.size(); ++i)
        {
            ...
            for (size_t k = 0; k < 2; ++k)
            {
                ...
                ON_CALL(*sensor, path())
                    .WillByDefault(ReturnStrView("path/" + std::to_string(i) +
                                                 "."s + std::to_string(k)));
        ...
        ON_CALL(*coreReport, metrics())
            .WillByDefault(ReturnRefOfCopy(coreMetrics));
    }
    ...
    std::shared_ptr<sdbusplus::asio::connection> bus =
        std::make_shared<decltype(bus)::element_type>(ioc);
    std::shared_ptr<sdbusplus::asio::object_server> objServer =
        std::make_shared<decltype(objServer)::element_type>(bus);

    std::shared_ptr<core::ReportMock> coreReport =
        std::make_shared<NiceMock<core::ReportMock>>();
    std::shared_ptr<core::JsonStorageMock> storage =
        std::make_shared<StrictMock<core::JsonStorageMock>>();
...
{
    EXPECT_CALL(*storage,
                list(core::interfaces::JsonStorage::Resource("Report")))
        .WillRepeatedly(
            Return(std::vector<core::interfaces::JsonStorage::Resource>(
...
{
    ASSERT_THAT(
        ReportConfiguration::path(
            core::interfaces::JsonStorage::Resource("tmp/Report/Domain/Name")),
...
{
    EXPECT_CALL(*storage, remove(core::interfaces::JsonStorage::Resource(
                              "Report/Domain/Name/configuration.json")));
...
{
    EXPECT_CALL(*storage, store(core::interfaces::JsonStorage::Resource(
                                    "Report/Domain/Name/configuration.json"),
                                _));
...
TEST_F(DBusReportTest, createsReportConfigurationFromGeneratedJson)
{
    ...
    EXPECT_THAT(
        reportConfiguration.metricParams,
        ElementsAre(MetricParams(api::SensorPaths({ObjPath("path/0.0"),
                                                   ObjPath("path/0.1")}),
                                 "AVERAGE", "Metric-0", "metadata-0"),
                    MetricParams(api::SensorPaths({ObjPath("path/1.0"),
                                                   ObjPath("path/1.1")}),
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(StoreParams{"name", nlohmann::json("Name")},
           ...
           StoreParams{
               "metricParams",
               nlohmann::json(
                   {MetricJson({ObjPath("path/0.0"), ObjPath("path/0.1")},
                               "AVERAGE"s, "Metric-0"s, "metadata-0"s),
                    MetricJson({ObjPath("path/1.0"), ObjPath("path/1.1")},
