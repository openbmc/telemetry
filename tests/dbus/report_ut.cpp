...
using MetricJson = utils::TupleWrapper<
    dbus::ReadingParam, utils::tstring::SensorPaths,
  ...
  public:
    ...
    void SetUp() override
    {
        std::transform(std::begin(metrics), std::end(metrics),
                       std::back_inserter(coreMetrics),
                       [](const auto& metric)
                           -> std::shared_ptr<core::interfaces::Metric> {
                           return metric;
                       });
        ...
        for (size_t i = 0; i < metrics.size(); ++i)
        {
            ...
            ON_CALL(*metrics[i], sensors())
                .WillByDefault(
                    ReturnSpan<std::shared_ptr<core::interfaces::Sensor>>(
                        sensors));
        }
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(
        StoreParams{"name", nlohmann::json("Name")},
