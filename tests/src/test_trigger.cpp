...
TEST_F(TestTrigger, setPropertyNameToMaxLengthValue)
{
    std::string name(TriggerManager::maxTriggerNameAndIdLength, 'z');
    EXPECT_THAT(setProperty(sut->getPath(), "Name", name),
                Eq(boost::system::errc::success));
    EXPECT_THAT(getProperty<std::string>(sut->getPath(), "Name"), Eq(name));
}

TEST_F(TestTrigger, DISABLED_failWhensetPropertyNameToLongerThanMaxLengthValue)
{
    std::string name(TriggerManager::maxTriggerNameAndIdLength + 1, 'z');
    EXPECT_THAT(setProperty(sut->getPath(), "Name", name),
