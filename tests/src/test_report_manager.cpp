...
TEST_F(TestReportManager, failToAddReportWithMoreSensorParametersThanExpected)
{
    reportFactoryMock.expectMake(std::nullopt, Ref(*sut), Ref(storageMock))
        .Times(0);
    ...
    auto metricParams = reportParams.metricParameters();
    auto& metricParamsVec = metricParams[0].at_index<0>();

    for (size_t i = 0; i < ReportManager::maxNumberMetrics; i++)
    {
        vmetricParamsVecec.emplace_back(LabeledSensorParameters{
