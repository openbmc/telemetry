    ...
    std::chrono::milliseconds interval)
{
    ...
    auto elapsed =
        utils::measureElapsedTime<std::chrono::milliseconds>([interval] {
            DbusEnvironment::waitForFuture("5th_async_read", interval * 10);
        });
...
TEST_F(TestSensor, notifiesInGivenIntervalAfterSchedule)
{
    ...
    // time frame.
    for (auto interval : {10, 250, 750})
    {
        auto elapsed = notifiesInGivenIntervalAfterSchedule(
                           std::chrono::milliseconds(interval))
                           .count();
        ASSERT_THAT(elapsed, Ge(interval * 5));
    ...
    ASSERT_THAT(notifiesInGivenIntervalAfterSchedule(1750ms).count(),
                AllOf(Ge(1750 * 5), Lt(1750 * 6)));
}
